...
function checkForLegalMove(clickDest) {
    ...
    // Move a card to the proper place if it's legal
...
function handleAceClick(element) {
    ...
    // if a face up card hasn't been clicked yet, pick and highlight this one
    if(!clickedCard && isFaceUpCard(element)){
        firstStackId = aceId;
        firstClickDest = clickDest;
        element.className += ' highlight';
...
function handleDrawClick(element) {
    ...
    // if there is no highlighted card and the draw pile isn't empty, select the top card
    ...
    // if the highlighted card is from the draw pile, put it back in the pile
